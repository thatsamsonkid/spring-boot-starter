...
public class ReactiveFetchPostsUseCase {
    ...
    public Mono<List<Post>> fetchPostsWithComments(List<String> postIds) {
        ...
        List<Mono<Post>> postMonos = postIds.stream().map(this::fetchPostWithComments).toList();

        // Combine all posts into a single list
        return Mono.zip(
                        postMonos,
                        results -> List.of(results).stream().map(result -> (Post) result).toList())
                .doOnSuccess(
                        posts ->
                                logger.info(
    ...
    private Mono<Post> fetchPostWithComments(String postId) {
        return externalApiPort
                ...
                .doOnError(
                        error ->
                                logger.error(
                                        "Failed to fetch post {} with comments", postId, error));
    }
}
